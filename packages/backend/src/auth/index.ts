...
const getGoogleAuth = (c: any) => new Google(
...
auth.get('/login/google', async (c) => {
  ...
  setCookie(c, "google_oauth_state", state, { path: "/", secure: true, httpOnly: true, maxAge: 600, sameSite: "Lax" })
  setCookie(c, "google_code_verifier", codeVerifier, { path: "/", secure: true, httpOnly: true, maxAge: 600, sameSite: "Lax" })
...
auth.get('/callback/google', async (c) => {
  ...
  try {
    ...
    const existingUser = await db.select().from(users).where(eq(users.email, userProfile.email)).get()
    ...
    await c.env.SESSIONS.put(`session:${sessionId}`, userId!, { expirationTtl: 60 * 60 * 24 * 30 }) // 30 days
